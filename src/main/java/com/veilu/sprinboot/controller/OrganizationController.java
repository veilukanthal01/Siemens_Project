...
public class OrganizationController {
	...
	public ResponseEntity createUser(@RequestBody Organization organization) {
		try {
			return new ResponseEntity<Organization>(this.organizationService.saveOrganization(organization),
					HttpStatus.OK);
	...
	/*
	 * @GetMapping("/organization/{orgId}/assets") public Set<Assets>
	 * getAssetsbyOrganization(@PathVariable long orgId){ return
	 * this.organizationService.getAssetsbyOrganization(orgId);
	 * 
	 * }
	 */
	...
	public ResponseEntity getAssetsbyOrganization(@PathVariable long orgId) {
		try {
			...
			if (!CollectionUtils.isEmpty(assetsList)) {
				...
				return new ResponseEntity<String>("Organization does not have any assets mapped", HttpStatus.NO_CONTENT);
			}
		} catch (OrganizationNotFoundException ex) {
			return new ResponseEntity<>(ex.getMessage(), HttpStatus.NOT_FOUND);
		}
